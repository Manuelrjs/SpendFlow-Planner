...
function LogoMarca() { return <span className="sf-brand-mark" aria-hidden="true"><Icono nombre="flujo" /></span>; }
...
export function NavegacionPrincipal() {
  ...
  const abrirEnlace = (evento: MouseEvent<HTMLAnchorElement>, enlace: Enlace) => {
  ...
  return <>
    <aside className="sf-sidebar">
      ...
      <nav className="sf-sidebar-nav" aria-label="Navegación principal">{gruposEnlaces.map((grupo) => <div key={grupo.titulo} className="sf-nav-group"><p>{grupo.titulo}</p><div>{grupo.enlaces.map((enlace) => <Link key={enlace.href} href={enlace.href} onClick={(evento) => abrirEnlace(evento, enlace)} className={`sf-nav-link ${estaActivo(enlace.href) ? 'is-active' : ''}`}><Icono nombre={enlace.icono} /><span>{enlace.etiqueta}</span></Link>)}</div></div>)}</nav>
    ...
    <nav className="sf-bottom-nav" aria-label="Navegación móvil"><ul>{enlacesPrincipales.map((enlace) => <li key={enlace.href}><Link href={enlace.href} onClick={(evento) => abrirEnlace(evento, enlace)} className={`${estaActivo(enlace.href) ? 'is-active' : ''} ${enlace.href === '/gastos/nuevo' ? 'is-primary' : ''}`}><Icono nombre={enlace.icono} /><span>{enlace.etiqueta === 'Nuevo gasto' ? 'Nuevo' : enlace.etiqueta === 'Flujo mensual' ? 'Flujo' : enlace.etiqueta}</span></Link></li>)}<li><button type="button" className={masAbierto || enlacesMas.some((enlace) => estaActivo(enlace.href)) ? 'is-active' : ''} onClick={() => setMasAbierto(true)} aria-haspopup="dialog" aria-expanded={masAbierto}><Icono nombre="mas" /><span>Más</span></button></li></ul></nav>
